...
public class TransactionController {
	...
	public ResponseEntity<TransactionResponse> deposit(@Valid @RequestBody TransactionRequest request,
		...
		return ResponseEntity.status(HttpStatus.CREATED).body(transactionService.deposit(username, request));
	}
	...
	public ResponseEntity<TransactionResponse> withdraw(@Valid @RequestBody TransactionRequest request,
			Authentication authentication) {
		String username = authentication.getName();
		return ResponseEntity.status(HttpStatus.CREATED).body(transactionService.withdraw(username, request));
	}
	...
	public ResponseEntity<TransactionResponse> transfer(@Valid @RequestBody TransactionRequest request,
			Authentication authentication) {
		String username = authentication.getName();
		return ResponseEntity.status(HttpStatus.CREATED).body(transactionService.transfer(username, request));
	}
	...
	@GetMapping("/{id}")
	public ResponseEntity<TransactionResponse> getMyTransaction(@PathVariable Long id, Authentication authentication) {
		String username = authentication.getName();
